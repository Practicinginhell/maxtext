...
def _qwen3_5_text_config(config):
  return config.get("text_config", config)


def _qwen3_5_hf_prefix(config):
  return "model.language_model" if "text_config" in config else "model"


def QWEN3_5_MAXTEXT_TO_HF_PARAM_MAPPING(config, maxtext_config, scan_layers=False):
  """Returns mapping from MaxText to HuggingFace Qwen3.5 text weight paths."""
  text_config = _qwen3_5_text_config(config)
  hf_base = _qwen3_5_hf_prefix(config)
  num_main_layers = text_config["num_hidden_layers"]
  num_experts = maxtext_config.num_experts
  layer_cycle_interval = maxtext_config.inhomogeneous_layer_cycle_interval

  mapping = {
      "params-token_embedder-embedding": f"{hf_base}.embed_tokens.weight",
      "params-decoder-decoder_norm-scale": f"{hf_base}.norm.weight",
      "params-decoder-logits_dense-kernel": "lm_head.weight",
  }

  def dense_mlp_paths(layer_prefixes):
    if isinstance(layer_prefixes, str):
      layer_prefixes = [layer_prefixes]
      scalar = True
    else:
      scalar = False

    def maybe_scalar(values):
      return values[0] if scalar else values

    return {
        "gate": maybe_scalar([f"{p}.input_layernorm.weight" for p in layer_prefixes]),
        "wi_0": [[f"{p}.mlp.gate_proj.weight" for p in layer_prefixes] for _ in range(num_experts)],
        "wi_1": [[f"{p}.mlp.up_proj.weight" for p in layer_prefixes] for _ in range(num_experts)],
        "wo": [[f"{p}.mlp.down_proj.weight" for p in layer_prefixes] for _ in range(num_experts)],
        "shared_wi_0": maybe_scalar([f"{p}.input_layernorm.weight" for p in layer_prefixes]),
        "shared_wi_1": maybe_scalar([f"{p}.input_layernorm.weight" for p in layer_prefixes]),
        "shared_wo": maybe_scalar([f"{p}.input_layernorm.weight" for p in layer_prefixes]),
        "shared_gate": maybe_scalar([f"{p}.input_layernorm.weight" for p in layer_prefixes]),
    }

  if scan_layers:
    for block_idx in range(layer_cycle_interval):
      hf_indices = list(range(block_idx, num_main_layers, layer_cycle_interval))
      hf_prefixes = [f"{hf_base}.layers.{i}" for i in hf_indices]
      prefix = f"params-decoder-layers-layer_{block_idx}"

      mapping[f"{prefix}-input_layernorm-scale"] = [f"{p}.input_layernorm.weight" for p in hf_prefixes]
      mapping[f"{prefix}-post_attention_layernorm-scale"] = [
          f"{p}.post_attention_layernorm.weight" for p in hf_prefixes
      ]
      ...
      if is_full_attention_layer:
        mapping.update(
            {
                f"{prefix}-attention-attention-query-kernel": [f"{p}.self_attn.q_proj.weight" for p in hf_prefixes],
                f"{prefix}-attention-attention-key-kernel": [f"{p}.self_attn.k_proj.weight" for p in hf_prefixes],
                f"{prefix}-attention-attention-value-kernel": [f"{p}.self_attn.v_proj.weight" for p in hf_prefixes],
                f"{prefix}-attention-attention-out-kernel": [f"{p}.self_attn.o_proj.weight" for p in hf_prefixes],
                f"{prefix}-attention-attention-query_norm-scale": [
                    f"{p}.self_attn.q_norm.weight" for p in hf_prefixes
                ],
                f"{prefix}-attention-attention-key_norm-scale": [f"{p}.self_attn.k_norm.weight" for p in hf_prefixes],
            }
        )
      else:
        mapping.update(
            {
                f"{prefix}-attention-in_proj_qkvz-kernel": [
                    (f"{p}.linear_attn.in_proj_qkv.weight", f"{p}.linear_attn.in_proj_z.weight")
                    for p in hf_prefixes
                ],
                f"{prefix}-attention-in_proj_ba-kernel": [
                    (f"{p}.linear_attn.in_proj_b.weight", f"{p}.linear_attn.in_proj_a.weight") for p in hf_prefixes
                ],
                f"{prefix}-attention-conv1d-kernel": [f"{p}.linear_attn.conv1d.weight" for p in hf_prefixes],
                f"{prefix}-attention-A_log": [f"{p}.linear_attn.A_log" for p in hf_prefixes],
                f"{prefix}-attention-dt_bias": [f"{p}.linear_attn.dt_bias" for p in hf_prefixes],
                f"{prefix}-attention-norm-rms_norm-scale": [f"{p}.linear_attn.norm.weight" for p in hf_prefixes],
                f"{prefix}-attention-out_proj-kernel": [f"{p}.linear_attn.out_proj.weight" for p in hf_prefixes],
            }
        )

      mlp = dense_mlp_paths(hf_prefixes)
      mapping.update(
          {
              f"{prefix}-mlp-routed_experts-gate-kernel": mlp["gate"],
              f"{prefix}-mlp-routed_experts-wi_0": mlp["wi_0"],
              f"{prefix}-mlp-routed_experts-wi_1": mlp["wi_1"],
              f"{prefix}-mlp-routed_experts-wo": mlp["wo"],
              f"{prefix}-mlp-shared_expert-wi_0-kernel": mlp["shared_wi_0"],
              f"{prefix}-mlp-shared_expert-wi_1-kernel": mlp["shared_wi_1"],
              f"{prefix}-mlp-shared_expert-wo-kernel": mlp["shared_wo"],
              f"{prefix}-mlp-shared_expert_gate-kernel": mlp["shared_gate"],
          }
      )
  else:
    for i in range(num_main_layers):
      hf_prefix = f"{hf_base}.layers.{i}"
      prefix = f"params-decoder-layers_{i}"
        ...
        mapping.update(
            {
                f"{prefix}-attention-in_proj_qkvz-kernel": (
                    ...
                    f"{hf_prefix}.linear_attn.in_proj_z.weight",
                ),
                f"{prefix}-attention-in_proj_ba-kernel": (
                    f"{hf_prefix}.linear_attn.in_proj_b.weight",
                    f"{hf_prefix}.linear_attn.in_proj_a.weight",
                ),
      ...
      mlp = dense_mlp_paths(hf_prefix)
      mapping.update(
          {
              f"{prefix}-mlp-routed_experts-gate-kernel": mlp["gate"],
              f"{prefix}-mlp-routed_experts-wi_0": mlp["wi_0"][0],
              f"{prefix}-mlp-routed_experts-wi_1": mlp["wi_1"][0],
              f"{prefix}-mlp-routed_experts-wo": mlp["wo"][0],
              f"{prefix}-mlp-shared_expert-wi_0-kernel": mlp["shared_wi_0"],
              f"{prefix}-mlp-shared_expert-wi_1-kernel": mlp["shared_wi_1"],
              f"{prefix}-mlp-shared_expert-wo-kernel": mlp["shared_wo"],
              f"{prefix}-mlp-shared_expert_gate-kernel": mlp["shared_gate"],
...
def QWEN3_5_MAXTEXT_TO_HF_PARAM_HOOK_FN(config, maxtext_config, scan_layers=False, saving_to_hf=False):
  """Transformation hooks for Qwen3.5 text checkpoints."""
  ...
  def concat_qkvz(input_tensors, target_shape):
    if saving_to_hf:
      qkv_shape, z_shape = target_shape
      hf_tensor = input_tensors.T
      qkv_size = qkv_shape[0]
      return [hf_tensor[:qkv_size].reshape(qkv_shape), hf_tensor[qkv_size:].reshape(z_shape)]
    qkv, z = input_tensors
    return np.concatenate([qkv, z], axis=0).T.reshape(target_shape)

  def concat_ba(input_tensors, target_shape):
    if saving_to_hf:
      b_shape, a_shape = target_shape
      hf_tensor = input_tensors.T
      b_size = b_shape[0]
      return [hf_tensor[:b_size].reshape(b_shape), hf_tensor[b_size:].reshape(a_shape)]
    b, a = input_tensors
    return np.concatenate([b, a], axis=0).T.reshape(target_shape)

  def permute_conv(input_tensor, target_shape):
    return input_tensor.transpose(2, 1, 0).reshape(target_shape)

  def zeros_like_target(input_tensor, target_shape):
    source = input_tensor[0] if isinstance(input_tensor, list) else input_tensor
    return np.zeros(target_shape, dtype=np.asarray(source).dtype)
  ...
  for i in loop_indices:
    ...
    if is_full_attention_layer:
      ...
      hooks[f"{prefix}-attention-in_proj_qkvz-kernel"] = concat_qkvz
      hooks[f"{prefix}-attention-in_proj_ba-kernel"] = concat_ba
      hooks[f"{prefix}-attention-out_proj-kernel"] = transpose
      hooks[f"{prefix}-attention-conv1d-kernel"] = permute_conv

    mlp_prefix = f"{prefix}-mlp"
    hooks[f"{mlp_prefix}-routed_experts-gate-kernel"] = zeros_like_target
    hooks[f"{mlp_prefix}-routed_experts-wi_0"] = transpose
    hooks[f"{mlp_prefix}-routed_experts-wi_1"] = transpose
    hooks[f"{mlp_prefix}-routed_experts-wo"] = transpose
    hooks[f"{mlp_prefix}-shared_expert-wi_0-kernel"] = zeros_like_target
    hooks[f"{mlp_prefix}-shared_expert-wi_1-kernel"] = zeros_like_target
    hooks[f"{mlp_prefix}-shared_expert-wo-kernel"] = zeros_like_target
    hooks[f"{mlp_prefix}-shared_expert_gate-kernel"] = zeros_like_target
...
PARAM_MAPPING = {
    ...
    "qwen3.5-2b": QWEN3_5_MAXTEXT_TO_HF_PARAM_MAPPING,
...
HOOK_FNS = {
    ...
    "qwen3.5-2b": QWEN3_5_MAXTEXT_TO_HF_PARAM_HOOK_FN,
